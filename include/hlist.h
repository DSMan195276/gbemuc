#ifndef INCLUDE_HLIST_H
#define INCLUDE_HLIST_H
...
#define hlist_entry_or_null(head, type, member) \
    ({ \
        __typeof__(head) __hlist_tmp = (head); \
...
#define hlist_foreach_entry(head, pos, member) \
    for (pos = hlist_entry_or_null((head)->first, __typeof__(*(pos)), member); \
         pos; \
         pos = hlist_entry_or_null((pos)->member.next, __typeof__(*(pos)), member))
